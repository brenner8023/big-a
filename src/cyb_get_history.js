...
async function main() {
  ...
  const limit = 100
  const dailyDataMap = {}
  if (!fs.existsSync(CACHE_CYB_DIR)) {
    fs.mkdirSync(CACHE_CYB_DIR)
  }
  for (let i = 0; i < chiNextStocks.length + limit - 1; i += limit) {
    queue.push(chiNextStocks.slice(i, i + limit))
  }
  ...
  Object.keys(dailyDataMap).forEach((tradeDate) => {
    const cacheFile = path.join(CACHE_CYB_DIR, `./${tradeDate}.json`)
